<?
	...
	include("opciones/seguridad.php");
				...
				?>	
					...
					function reporte(){
					//document.form1.boton3.value="Regresas";
					window.open("reportes/R_Vehiculo.php","mywindow","location=1,status=1,scrollbars=1,width=100,height=100");
					//document.location.href = ('inicio.php');
					//document.form1.href = ('inicio.php'); 
...
include("opciones/conexion_prestamo.php") ;//conexPres 
					$b = mysql_query ("SELECT COUNT(*) as registros FROM analisis_vehiculo WHERE id_analisis_vehiculo='$cedulaS' and status_Analisis_vehiculo='Activa'  ",$conexPres);
					while($v=mysql_fetch_assoc($b))	
					{
					 echo $n_registro=1+$v['registros']; 
			...
			{	
				...
				$consulta=mysql_query(" INSERT INTO analisis_vehiculo (
`cod_analisis_vehiculo` ,
`id_analisis_vehiculo` ,
`monto` ,
`cut_mensual` ,
`seg_incendio` ,
`seg_vida` ,
...
`status_Analisis_vehiculo` 
 ...
 else if($_POST['boton1']=="Calcular")
			{
					...
					$cuota_seguro=(($monto*$tasa_hipo2)/100)*100;
					/*------------------------fin-----------------------------------*/
						//echo ('-'),$cuota_seguro; 
					...
					*/
					...
					/*----------------calculo de analisis------------- */
							...
							$monto_incendio;
							$cuota_seguro;
							$total_cuota_seguro=$cuota_seguro*0.08884879;
...
<div>
 ...
 <form id="form1" name="form1" method="post" action="Solicitud_Credito_vehiculo4.php">
...
<!------------------------------------------------------------------------------------------------------------------------------------>			
			<table  colspan="4" align="center" >	
				...
				<tr>
				  <td colspan="4" id="titulo2" align="center" bgcolor="#FFFFFF">Calculadora</td>
				</tr>
				...
				<tr>
					<td>
							...
							$b = mysql_query ("SELECT * FROM tipo_prestamos WHERE cod_tipo_prestamo = '$tipo_prestamo'  ",$conexPres);
								while($v=mysql_fetch_assoc($b))	{  $plazo=$v['numeros_cuotas']/12;  }	
							*/
							$servidor="127.0.0.1";
							$usuario="root";
							$clave="1234";
							$db_nombre="administrador";


							$conexadmi=@mysql_connect($servidor,$usuario,$clave) or die(mysql_error());
							@mysql_select_db($db_nombre,$conex) or die(mysql_error()); ?>
								...
								<select  name="plazo"   id="plazo" >
								  ...
								  <option required value="<?php echo $vec1[1];?>"> <?php echo $vec1[0];  ?> </option>;
								  <?php } ?>
								  </select>
					</td>
				</tr>
				...
				<tr>   
					<td colspan=4 align="center" >
						...
						<input type="hidden" name="d2" value="<?echo $cuota_mensual;?>" >
...
<table BORDER=1 colspan="9">	
				<tr>
				  <td colspan="9" id="titulo3" align="center" bgcolor="#FFFFFF">Calculo Para El Prestamo</td>
				</tr>
				<tr>
		<td rowspan=2> </td>
		<td align="center" colspan=2>CRÉDITO</td>
		<td align="center" colspan=2>SEGURO</td>
	...
	<tr>
		...
		<td colspan=1>Bs<?echo number_format($cuota_mensual, 2, ",", "."); //round($cuota2,2) //$cuota2;?></td>
	...
	$b = mysql_query ("SELECT COUNT(*) as registros FROM   analisis_vehiculo WHERE id_analisis_vehiculo='$cedulaS' and status_Analisis_vehiculo='Activa'  ",$conexPres);
		...
		$b = mysql_query ("SELECT * FROM analisis_vehiculo WHERE id_analisis_vehiculo='$cedulaS' and status_Analisis_vehiculo='Activa' and opcion='1'  ",$conexPres);
					while($v=mysql_fetch_assoc($b))	
					{
					 $monto=$v['monto']; 
	...
	<?} if($n_registro>2){ 
	include("opciones/conexion_prestamo.php"); //conexPres
		$b = mysql_query ("SELECT * FROM analisis_vehiculo WHERE id_analisis_vehiculo='$cedulaS' and status_Analisis_vehiculo='Activa' and opcion='2'  ",$conexPres);
					while($v=mysql_fetch_assoc($b))	
					{
					 $monto=$v['monto']; 
					  $cut_mensual=$v['cut_mensual']; 
	...
	include("opciones/conexion_prestamo.php"); //conexPres
		$b = mysql_query ("SELECT * FROM analisis_vehiculo WHERE id_analisis_vehiculo='$cedulaS' and status_Analisis_vehiculo='Activa' and opcion='3'  ",$conexPres);
					while($v=mysql_fetch_assoc($b))	
					{
					 $monto=$v['monto']; 
					  $cut_mensual=$v['cut_mensual']; 
