...
function guardar(){
	...
	else if(document.form1.montaval.value.length==0){
		alert("Defina  Monto de Avaluo");
		document.form1.montaval.focus();
	} 
	else if(document.form1.Perito.value.length==0){
		alert("Defina  Monto de Perito");
		document.form1.Perito.focus();
	} 
...
$consulta1=mysql_query(" UPDATE solicitudes SET `monto_avaluo` = '$montaval' , `Honorarios_perito` = '$Perito'  , `observaciones` = '$obs1 $ob'     WHERE cod_solicitudes ='$cod_solicitudes' ;",$conexPres);			
...
VALUES ('null', '$cedulaS', '$cod_solicitudes', '$usunom $usuapel', 'Realizo la solicitud crédito observaciones:$obs', '$nombre_pc', '$id_usu', 'Crédito') ",$conexadmi);
				...
				if($consulta1){
								echo "<script type='text/javascript'> alert('Su Solicitud ha sido enviada satisfactoriamente, recuerde imprimir la planilla y enviar junto con los recaudos correspondientes');document.location.href = ('Solicitud_Credito_Hipotecario3.php'); </script>";
								}
						else	{
...
include("opciones/conexion.php");
 ...
 $personas =mysql_query("SELECT Nom_Ape, nombre1, nombre2, apellido1, apellido2, nacionalidad, cedula, sexo, N_cuenta, F_nacimiento, correo, nom_estatus, tipo_nomina, nom_organismo, est_civil, direccion, estado, municipio, parroquia, tipo_vivienda
FROM  `socios` AS soc
...
?>
 ...
 <form id="form1" name="form1" method="post" action="Solicitud_Credito_Hipotecario2.php">
			...
			<table align="center" border="0" cellpadding="0" cellspacing="0">
				<tr>
				  <td  id="titulo2" colspan="3" align="center" >Datos para la  Póliza de Seguro  Hipotecario </td>
				</tr>
				<tr>
					<td>Peso:</td>
				...
				<tr>
					...
					<td>
						<select  name="vivi"   id="v"  >
							...
							<option required value="Casa">Casa</option>;
							<option required value="Apartamento">Apartamento</option>;
						</select>
					</td>
				</tr>
				
				<tr>
					<td>Monto del Avaluo:</td>
					<td> <input type="text" name="montaval" size="18" maxlength="9"  onKeyPress="return permite(event,'num')" > </td>
				</tr>	
				<tr>
					<td>Cobro Perito:</td>
					<td> <input type="text" name="Perito" size="18" maxlength="9"  onKeyPress="return permite(event,'num')" > </td>
				</tr>
				<tr>
					<td>
						Observaciones: 
					...
					<td colspan=4>
					<textarea name="obs1"  size="20" maxlength="100" value="" onKeyPress="return permite(event,'car')" rows="2" cols="44"> </textarea>
