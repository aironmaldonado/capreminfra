...
<?		
		...
		$f_nacimiento = explode("-",$f);
			...
			$sql="Select * from cheques where bene LIKE  '%$p%' and fecha = CTOD('$mmN/$ddN/$aaaaN')";
			...
			include("opciones/conexion_prestamo.php");
				...
				$sql="Select * from cheques where  solicitud='$solicitud'";
			...
			$result=odbc_exec($conexion,$sql)or die(exit("Error en odbc_exec"));
						...
						?><td align="center" ><?echo $afianzado=odbc_result($result,"cedula"); ?></td><?
...
//Pago Proveedor/KEVES VISION, C.A12012015.txt
				$name12="Pago Proveedor/$p$d$m$a.txt"; 
				$name13="$p$d$m$a.txt"; 
				?>
</div>
