...
if($buscar){
		...
		$resultados = mysql_query("SELECT * FROM `solicitudes`  where status_solicitud='Procesando' and id_solicitudes = '$buscar' ",$conexPres); 
		
		$total_registros = mysql_num_rows($resultados); 
		$resultados = mysql_query("  SELECT * FROM `solicitudes`   where id_solicitudes = '$buscar' and status_solicitud='Procesando' and cod_solicitudes > '0' ORDER BY cod_solicitudes ASC LIMIT $inicio, $registros",$conexPres);	
			 
			} 
else {
		$resultados = mysql_query("SELECT * FROM `solicitudes`  where status_solicitud='Procesando'  ",$conexPres); 
		$total_registros = mysql_num_rows($resultados); 
		$resultados = mysql_query("  SELECT * FROM `solicitudes`   where  status_solicitud='Procesando' and cod_solicitudes > '0' ORDER BY cod_solicitudes ASC LIMIT $inicio, $registros",$conexPres);	
			...
			while ($pacientes = mysql_fetch_array($resultados)) {
				...
				<tr  onClick="ver_detalles('<? echo $pacientes["id_solicitudes"]; ?>')" style="cursor:pointer" onMouseMove="this.style.backgroundColor='#0a8df2'" onMouseOut="this.style.backgroundColor='#BFDFFF'" >
					  ...
					  <td align="center">	<a href= "Status_Solicitudes.php?CI=<?echo $pacientes["id_solicitudes"];?>& codigo=<?echo $pacientes["cod_solicitudes"];?>"><? echo $pacientes["status_solicitud"]; ?></a></td>
					  <td align="center">	<a href= "Listado_Solicitudes1.php?CI=<?echo $pacientes["id_solicitudes"];?>&credito=<?echo $tipo_prestamo1;?> & cod_soli=<?echo $pacientes["cod_solicitudes"];?>"><img src="Imagen\iconos_tablas\estatus.jpg"></a></td>
				</tr>	
